...
import datetime
# 打开数据库连接
db = pymysql.connect("172.18.2.21", "omniv5", "ABCabc123", "omniv5_wms",  charset='utf8')
...
try:
   ...
   # sql3 = str(dateList).replace("\'NULL\'", 'NULL').replace("[","(").replace("]",");")
    ...
    sql3 = str(dateList).replace("None", "NULL").replace(
        "[", "(").replace("]", ");")
    ...
    #print("values =",values)
    sql1 = "insert into outboundorder('%s') VALUES " % ("','".join(field_names))
    #print(sql1)
